...
class NFLFavTeamNextGameScene(FavTeamNextGameScene):
    """ Favourite team next game scene for the NFL. Contains functionality to pull schedule data from ESPN API, parse, and build+display images based on the result.
    ...
    def __init__(self):
        ...
        First runs init from the generic FavTeamNextGameScene class.
        """
    ...
    def display_scene(self):
        ...
        if self.favourite_teams:
            for team in self.favourite_teams:
                next_game_details = data.nfl_data.get_next_game(team)

                if next_game_details:
                    # If a game is in progress, and display_if_in_progress is False, skip.
