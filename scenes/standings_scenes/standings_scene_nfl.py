...
class NFLStandingsScene(StandingsScene):
    """ Standings scene for the NFL. Contains functionality to pull standings data from the ESPN API, process as needed, and build+display images based on the result.
    ...
    def __init__(self):
        ...
        First runs init from the generic StandingsScene class.
        """
    ...
    def display_scene(self):
        ...
        self.data = {
            'standings': data.nfl_data.get_standings()
        }
        ...
        for type in self.settings['display_for']:
            # Divisions.
            if type == 'division':
                for div in self.data['standings']['division']['divisions'].items():
                    div_details = div[1]
            ...
            # Conferences.
                ...
                for conf in self.data['standings']['conference']['conferences'].items():
                    conf_details = conf[1]
                    self.build_standings_image('conference', conf_details['abrv'], conf_details['teams'], playoff_cutoff_hard=self.data['standings']['conference']['playoff_cutoff_hard'])
                    self.display_standing_images()
